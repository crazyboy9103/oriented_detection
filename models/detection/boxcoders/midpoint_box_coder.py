...
def decode_midpoint_boxes(pred_bboxes: Tensor, bboxes: Tensor, weights: Tensor, bbox_xform_clip: float) -> Tensor:
    ...
    polys = torch.stack([pred_a, y1, x2, pred_b, _pred_a, y2, x1, _pred_b], dim=-1) # (N, 8)
    center = torch.stack([pred_ctr_x, pred_ctr_y, pred_ctr_x, pred_ctr_y, pred_ctr_x, pred_ctr_y, pred_ctr_x, pred_ctr_y], dim=-1) # (N, 8)
    center_polys = polys - center
    
    # shorter diagonal to match longer diagonal
    diag_len = torch.sqrt(center_polys[:, 0::2] * center_polys[:, 0::2] + center_polys[:, 1::2] * center_polys[:, 1::2])
