...
from models.detection.boxcoders import MidpointBoxCoder
...
def concat_box_prediction_layers(box_cls: List[Tensor], box_regression: List[Tensor]) -> Tuple[Tensor, Tensor]:
    ...
    for box_cls_per_level, box_regression_per_level in zip(box_cls, box_regression):
        ...
        box_regression_per_level = permute_and_flatten(box_regression_per_level, N, A, 6, H, W)
        box_regression_flattened.append(box_regression_per_level)
    # concatenate on the first dimension (representing the feature levels), to
    # take into account the way the labels were generated (with all feature maps
    # being concatenated as well)
    box_cls = torch.cat(box_cls_flattened, dim=1).flatten(0, -2)
    box_regression = torch.cat(box_regression_flattened, dim=1).reshape(-1, 6)
    return box_cls, box_regression


class OrientedRegionProposalNetwork(nn.Module):
    """
    ...
    def forward(
        ...
        proposals = proposals.view(num_images, -1, 5) # oriented proposals
        boxes, scores = self.filter_proposals(proposals, objectness, images.image_sizes, num_anchors_per_level)

        # training target is (..., 6) shape 
        losses = {}
